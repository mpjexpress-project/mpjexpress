...
for(int i=0 ; i<100 ; i=i+8) {
 ...
 req[1]=MPI.COMM_WORLD.Isend(byteArray,0,DATA_SIZE,MPI.BYTE,1,i+1);
 req[2]=MPI.COMM_WORLD.Isend(charArray,0,DATA_SIZE,MPI.CHAR,1,i+2);
 req[3]=MPI.COMM_WORLD.Isend(doubleArray,0,DATA_SIZE,MPI.DOUBLE,1,i+3);
 req[4]=MPI.COMM_WORLD.Isend(longArray,0,DATA_SIZE,MPI.LONG,1,i+4);
 req[5]=MPI.COMM_WORLD.Isend(booleanArray,0,DATA_SIZE,MPI.BOOLEAN,1,i+5);
 req[6]=MPI.COMM_WORLD.Isend(shortArray,0,DATA_SIZE,MPI.SHORT,1,i+6);
 req[7]=MPI.COMM_WORLD.Isend(floatArray,0,DATA_SIZE,MPI.FLOAT,1,i+7);
...
req[0]=	MPI.COMM_WORLD.Irecv(intReadArray,0,DATA_SIZE,MPI.INT,0,i);
req[1]=	MPI.COMM_WORLD.Irecv(byteReadArray,0,DATA_SIZE,MPI.BYTE,0,i+1);
req[2]=	MPI.COMM_WORLD.Irecv(charReadArray,0,DATA_SIZE,MPI.CHAR,0,i+2);        	req[3]=	MPI.COMM_WORLD.Irecv(doubleReadArray,0,DATA_SIZE,MPI.DOUBLE,0,i+3);    	
req[4]=	MPI.COMM_WORLD.Irecv(longReadArray,0,DATA_SIZE,MPI.LONG,0,i+4);        	
req[5]=	MPI.COMM_WORLD.Irecv(booleanReadArray,0,DATA_SIZE,MPI.BOOLEAN,0,i+5);  	
req[6]=	MPI.COMM_WORLD.Irecv(shortReadArray,0,DATA_SIZE,MPI.SHORT,0,i+6);      	
req[7]=	MPI.COMM_WORLD.Irecv(floatReadArray,0,DATA_SIZE,MPI.FLOAT,0,i+7);
