...
*/
    ...
    if (myrank == src) {
      buffer = new mpjbuf.Buffer(
                   BufferFactory.create(BUF_SIZE+MPI.SEND_OVERHEAD),
                   MPI.SEND_OVERHEAD, BUF_SIZE+MPI.SEND_OVERHEAD );
      ...
      buffer = new mpjbuf.Buffer(
                    BufferFactory.create(BUF_SIZE + MPI.RECV_OVERHEAD),
                    MPI.RECV_OVERHEAD, BUF_SIZE+MPI.RECV_OVERHEAD );
    ...
    if(myrank == 1)  System.out.println("PackO TEST COMPLETE");    
    MPI.Finalize();
