  ...
  public BufferTest3(String args[]) throws Exception {
    ...
    if (MPJDev.WORLD.id() == 0) {
      ...
      Buffer charBuffer = new Buffer(
		      BufferFactory.create( 80 + 8 + SEND_OVERHEAD), 
		      SEND_OVERHEAD, 80+8+SEND_OVERHEAD );
      charBuffer.putSectionHeader(Type.CHAR);
      charBuffer.gather(charArray, 40, 0, indexes);
      charBuffer.commit();

      Buffer intBuffer = new Buffer(
	 	      BufferFactory.create( 160 + 8 + SEND_OVERHEAD), 
		      SEND_OVERHEAD, 160+8+SEND_OVERHEAD );
      ...
      Buffer longBuffer = new Buffer(
		      BufferFactory.create( 320 + 8 + SEND_OVERHEAD), 
		      SEND_OVERHEAD, 320+8+SEND_OVERHEAD );
      ...
      Buffer intBuffer = new Buffer(
		      BufferFactory.create( 160 + 8 + RECV_OVERHEAD), 
		      RECV_OVERHEAD, 160+8+RECV_OVERHEAD );
      ...
      Buffer doubleBuffer = new Buffer(
		      BufferFactory.create( 320 + 8 + RECV_OVERHEAD), 
		      RECV_OVERHEAD, 320+8+RECV_OVERHEAD );
      ...
      Buffer charBuffer = new Buffer(
		      BufferFactory.create( 80 + 8 + RECV_OVERHEAD), 
		      RECV_OVERHEAD, 80+8+RECV_OVERHEAD );
