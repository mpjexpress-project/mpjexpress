...
public class JGFiAlltoallBench implements JGFSection1{
  ...
  public JGFiAlltoallBench(int rank, int nprocess) {
	this.rank = rank;  
...
/* Carryout the broadcast operation */
        for (int k=0; k<size; k++){
          req = MPI.COMM_WORLD.Ialltoall(send_arr,0,m_size,MPI.DOUBLE,
...
/* Carryout the broadcast operation */
        for (int k=0; k<size; k++){
          req = MPI.COMM_WORLD.Ialltoall(send_arr_obj,0,m_size,MPI.OBJECT,recv_arr_obj,0,m_size,MPI.OBJECT);
          req.Wait();
          MPI.COMM_WORLD.Barrier();
...
/* determine the cost of the Barrier, subtract the cost and write out the performance time */
  ...
  public static void main(String[] argv) throws Exception{
         JGFiAlltoallBench p150 = new JGFiAlltoallBench(argv);

  }

  public JGFiAlltoallBench(String[] argv) throws Exception {
...
/* Initialise MPI */
     ...
     JGFiAlltoallBench ata = new JGFiAlltoallBench(rank, nprocess);
     ata.JGFrun();
