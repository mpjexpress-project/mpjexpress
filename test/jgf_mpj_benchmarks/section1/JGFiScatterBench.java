...
public class JGFiScatterBench implements JGFSection1{
  ...
  public JGFiScatterBench(int rank, int nprocess) {
	this.rank = rank;   
...
/* Carryout the broadcast operation */
        for (int k=0; k<size; k++){
          req = MPI.COMM_WORLD.Iscatter(send_arr,0,recv_arr.length,MPI.DOUBLE,recv_arr,0,recv_arr.length,MPI.DOUBLE,0);
          req.Wait();
          MPI.COMM_WORLD.Barrier();
...
/* Carryout the broadcast operation */
        for (int k=0; k<size; k++){
          req = MPI.COMM_WORLD.Iscatter(send_arr_obj,0,recv_arr_obj.length,MPI.OBJECT,recv_arr_obj,0,recv_arr_obj.length,MPI.OBJECT,0);
          req.Wait();
          MPI.COMM_WORLD.Barrier();
...
/* determine the cost of the Barrier, subtract the cost and write out the performance time */
  ...
  public static void main(String[] argv) throws Exception{
     JGFiScatterBench p150 = new JGFiScatterBench(argv);
  }

  public JGFiScatterBench(String[] argv) throws Exception {
...
/* Initialise MPI */
     ...
     JGFiScatterBench sc = new JGFiScatterBench(rank, nprocess);
     sc.JGFrun();
