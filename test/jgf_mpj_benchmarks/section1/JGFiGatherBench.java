...
public class JGFiGatherBench implements JGFSection1{
  ...
  public JGFiGatherBench(int rank, int nprocess) {
	this.rank = rank;   
...
/* Carryout the broadcast operation */
        for (int k=0; k<size; k++){
          req = MPI.COMM_WORLD.Igather(send_arr,0,send_arr.length,MPI.DOUBLE,recv_arr,0,send_arr.length,MPI.DOUBLE,0);
          req.Wait();
          MPI.COMM_WORLD.Barrier();
...
/* Carryout the broadcast operation */
        for (int k=0; k<size; k++){
          req=MPI.COMM_WORLD.Igather(send_arr_obj,0,send_arr_obj.length,MPI.OBJECT,recv_arr_obj,0,send_arr_obj.length,MPI.OBJECT,0);
          req.Wait();
          MPI.COMM_WORLD.Barrier();
...
/* determine the cost of the Barrier, subtract the cost and write out the performance time */
  ...
  public static void main(String[] argv) throws Exception{
    ...
    JGFiGatherBench p150 = new JGFiGatherBench(argv);
  }

  public JGFiGatherBench(String[] argv) throws Exception {
...
/* Initialise MPI */
     ...
     JGFiGatherBench ga = new JGFiGatherBench(rank, nprocess);
     ga.JGFrun();
