...
public class JGFiBcastBench implements JGFSection1{
  ...
  public JGFiBcastBench(int rank, int nprocess) {
	this.rank = rank;  
...
/* Carryout the broadcast operation */
        for (int k=0; k<size; k++){
          req = MPI.COMM_WORLD.Ibcast(arr,0,arr.length,MPI.DOUBLE,0);
          req.Wait();
          MPI.COMM_WORLD.Barrier();
        }
...
/* Broadcast time to the other processes  */
        MPI.COMM_WORLD.Barrier();
        req = MPI.COMM_WORLD.Ibcast(time,0,1,MPI.DOUBLE,0);
        req.Wait();
        size *=2;
      }
...
/* determine the cost of the Barrier, subtract the cost and write out the performance time */
  ...
  public static void main(String[] argv) throws Exception{
    JGFiBcastBench p150 = new JGFiBcastBench(argv);
  }

  public JGFiBcastBench(String[] argv) throws Exception {
...
/* Initialise MPI */
     ...
     JGFiBcastBench bc = new JGFiBcastBench(rank, nprocess);
     bc.JGFrun();
